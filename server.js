...
expressApp.post('/webhook/whatsapp', async (req, res) => {
  try {
    ...
    // Process WhatsApp message similar to Telegram
    const text = incomingMsg.toLowerCase();
    const userId = from.replace('whatsapp:', '');
    
    // Handle commands
    if (text === 'start' || text === 'menu') {
      await sendWhatsAppMessage(from, `📋 *Welcome to Kwasu Lost And Found Bot!*\n_v0.1 Designed & Developed by_ Rugged of ICT.\n\nTo proceed with, Select what you are here for from the menu:\n\n1. *Report Lost Item*\n2. *Report Found Item*\n3. *Search for my lost Item*\n4. *Contact Developer*\n\nKindly Reply with 1, 2, 3, or 4.`);
    } 
    // Report lost
    else if (text === '1') {
      await sendWhatsAppMessage(from, '🔍 *Report Lost Item*\n\nPlease provide the following details:\nITEM, LOCATION, DESCRIPTION\n\nExample: "Water Bottle, Library, Blue with sticker"');
      await set(ref(db, `users/${userId}`), { action: 'report_lost' });
    }
    // Report found
    else if (text === '2') {
      await sendWhatsAppMessage(from, '🎁 *Report Found Item*\n\nPlease provide the following details:\nITEM, LOCATION, CONTACT_PHONE\n\nExample: "Keys, Cafeteria, 08012345678"');
      await set(ref(db, `users/${userId}`), { action: 'report_found' });
    }
    // Search
    else if (text === '3') {
      await sendWhatsAppMessage(from, '🔎 *Search for my lost Item*\n\nPlease reply with a keyword to search:\n\nExample: "water", "keys", "bag"');
      await set(ref(db, `users/${userId}`), { action: 'search' });
    }
    // Contact developer
    else if (text === '4') {
      await sendWhatsAppMessage(from, `📞 *Contact Developer*\n\nFor any issues or support, please contact the developer:\n\n*WhatsApp:* 09038323588\n\n*Note:* Please go straight to the point in your DM to avoid late response. Be direct and clear about your issue or inquiry.`);
    }
    // Handle responses
    else {
      await handleWhatsAppResponse(userId, text, from);
    }
...
async function handleWhatsAppResponse(from, msg, to) {
  try {
    // Get user state
    const userSnapshot = await get(child(ref(db), `users/${from}`));
    const user = userSnapshot.val();
    ...
    if (!user) {
      await sendWhatsAppMessage(to, '❓ Invalid command. Reply "menu" for options.');
      return;
    }
    ...
    if (user.action === 'report_lost' || user.action === 'report_found') {
      const parts = msg.split(',');
      if (parts.length < 3) {
        await sendWhatsAppMessage(to, `⚠️ Format error. Please use: ${user.action === 'report_lost' ? 'ITEM, LOCATION, DESCRIPTION' : 'ITEM, LOCATION, CONTACT_PHONE'}`);
        return;
      }
      ...
      if (user.action === 'report_lost') {
        ...
        confirmationMsg += `🙏 *Thank you for using KWASU Lost & Found Bot!*`;
        await sendWhatsAppMessage(to, confirmationMsg);
      } else {
        let confirmationMsg = `✅ *Found Item Reported Successfully!*\n\n`;
        confirmationMsg += `📦 *Item:* ${item}\n`;
        confirmationMsg += `📍 *Location:* ${location}\n`;
    ...
    // Handle search
      ...
      if (!reports || Object.keys(reports).length === 0) {
        await sendWhatsAppMessage(to, '❌ No items found in the database.');
        return;
      }
    ...
    console.error('Handle response error:', error);
    await sendWhatsAppMessage(to, '❌ An error occurred. Please try again.');
  }
}
...
async function setWebhook() {
  try {
    const url = `${process.env.RENDER_URL}/webhook/${TELEGRAM_TOKEN}`;
